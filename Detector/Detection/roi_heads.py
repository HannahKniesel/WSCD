...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # stack for soft labels on two classes with propabilities
    classification_loss = F.cross_entropy(class_logits, torch.stack([1-labels, labels], axis=-1))
    # classification_loss = F.binary_cross_entropy_with_logits(class_logits, torch.stack([1-labels, labels], axis=-1)) #torch.stack([labels, 1-labels], axis=-1))


    # get indices that correspond to the regression targets for
    # the corresponding ground truth labels, to be used with
    # advanced indexing
    # convert labels back to int64 for BB regression loss such that labels > 0 are no BG
    labels = (labels>0).to(torch.int64)
...
class RoIHeads(nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                labels_in_image = torch.zeros((proposals_in_image.shape[0],), device=device)
    ...
    def subsample(self, labels):
        ...
        labels_binary = [(l>0).to(torch.int64) for l in labels] # convert labels such that labels > 0 are no BG
        sampled_pos_inds, sampled_neg_inds = self.fg_bg_sampler(labels_binary) 
        sampled_inds = []
